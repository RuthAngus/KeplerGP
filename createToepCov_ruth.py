...
# This version has been adapted for M = 1
...
# sv vector which contain the lenght of vector C and T, N[0]=size(T0),size(C0)...
    ...
    d=np.zeros((M,1))
    for i in range(0,M):
        Te=T[n:n+sv[i]:] # FIXME: when you go back to stacking, Te=T[n:n+sv[i],:]
        Ce=C[n:n+sv[i]:]
        Ye=Y[n:n+sv[i]:]
        T_nan=np.isnan(Te)
        Y_nan=np.isnan(Ye)
        ind1=np.where(T_nan==False)
        ind2=np.where(Y_nan==False)
        if i==0 :
            Cf=Ce
            Yf=Ye
            N=np.shape(Cf)[0]
    ...
    inds=np.intersect1d(inds1[0],inds2[0])
